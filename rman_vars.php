...
include_once("rman_formclasses.inc");
  ...
  if (isset($_POST['showvid_submit']) && !$handled) {
    $svsubmit=SanitizeUserInput($_POST['showvid_submit'],"plaintext");
    if ($debug) print "<BR>$svsubmit\n";
    
    $vid=SanitizeUserInput($_GET['vid'],"int",11,$modified);
	...
	break;
      ...
      case "DeleteSelected":
	if (!$modified) vars_sv_delete($vid,$_POST['showvid_select']);
	break;

      case "Update":
        if (!$modified) vars_updatedefaults($vid,$_POST['defvalue'],$_POST['defcomment']);
        break;
	...
	break;
      ...
      case "UpdateSensors":
        if (!$modified) vars_updatesenvars($vid,$_POST['vid_val'],$_POST['vid_com']);
        break;
  ...
  if (isset($_POST['create_submit']) && !$handled) {
    ...
    switch ($createsubmit) {
      case "Create":
        $handled=vars_createnew($_POST['name'],$_POST['value'],$_POST['comment']);
        break;
  ...
  if (isset($_POST['main_submit']) && !$handled) {
	...
	$handled=true;
      break;
      case "DeleteSelected":
        if (isset($_POST['vars_select'])) vars_maindelselect($_POST['vars_select']);
        break; 
      default:
      PrintError("Invalid Submission Received");
    }
...
function vars_updatedefaults($vid,$value,$comment) {
  ...
  if (!$modified) {
    if (($value != $defaults->value) || ($comment != $defaults->comment)) {
      ...
      RunQuery($senslist,"SELECT rman_sensor.sid FROM rman_sensor LEFT JOIN rman_varvals ON (rman_sensor.sid=rman_varvals.sid AND vid='".$vid."') WHERE rman_varvals.sid IS NULL");
...
function vars_update($vid,$sid,$value,$comment) {
  RunQuery($result,"UPDATE rman_varvals SET value='".mysqli_escape_string($value)."', comment='".mysqli_escape_string($comment)."' WHERE vid='".$vid."' AND sid='".$sid."'");
}
...
function vars_sv_sessinit(&$vid,&$sv_sensors,&$sv_defdata,&$sv_senvars,&$ses_restored=false) {
  global $_SESSION;
  $ses_restored=false;
  if (isset($_SESSION["s_vid"])) {
    $s_vid=$_SESSION["s_vid"];
    if ($s_vid == $vid) {
...
function vars_showvid($vid) {
  ...
  print "<A HREF='{$_SERVER['PHP_SELF']}'>Return to Variable Maintenance</A>\n";

  $defaults = new html_tableform("var_view","post","{$_SERVER['PHP_SELF']}?vid=".$vid);
...
function vars_createnew($name, $value, $comment) {
  ...
  RunQuery($result,"INSERT INTO rman_varvals (vid,sid,value,comment) VALUES ('".$vid."', 0, '".mysqli_escape_string($value)."','".mysqli_escape_string($comment)."')");
  # Now update sensors ....
  UpdateAllActiveSensors();
  return(false);
}
...
function vars_showmain() {
  ...
  $vars = new html_tableform("var_summary","post","{$_SERVER['PHP_SELF']}");
  ...
  While ($row = mysqli_fetch_object($result)) {
    $vars->AddRowElem(new html_RowElem("&nbsp;<font><A HREF='{$_SERVER['PHP_SELF']}?vid=" . $row->vid . "'>".$row->vname."</font>&nbsp;","LEFT"));
